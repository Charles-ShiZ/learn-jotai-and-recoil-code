...
var createStore = (initialValues) => {
  const committedAtomStateMap = new WeakMap();
  ...
  const suspensePromiseCacheMap = new WeakMap();
  ...
  const setAtomState = (version, atom2, atomState) => {
    ...
    if (version) {
      ...
      if (!pendingMap.has(atom2)) {
        ...
        // console.log(atom2, pendingMap.size, new Map(Array.from(pendingMap)))
        // console.log('pendingMap', pendingMap)
      }
    }
  };
  const createReadDependencies = (version, prevReadDependencies = new Map(), dependencies) => {
    if (!dependencies) {
    ...
    // console.log(dependencies, readDependencies)
  ...
  const setAtomValue = (version, atom2, value, dependencies, suspensePromise) => {
    ...
    const nextAtomState = {
      v: value,
      r: (atomState == null ? void 0 : atomState.r) || 0,  // 疑似读取次数
      d: createReadDependencies(version, atomState == null ? void 0 : atomState.d, dependencies) // 依赖的atoms
    };
    // 首次加载 atom 走上面
  ...
  const setAtomSuspensePromise = (version, atom2, suspensePromise, dependencies) => {
    const atomState = getAtomState(version, atom2);
    console.log('旧atomState', atomState)
    // 当旧数据也是Promise的时候
  ...
  const setAtomPromiseOrValue = (version, atom2, promiseOrValue, dependencies) => {
    if (promiseOrValue instanceof Promise) {
      const suspensePromise = createSuspensePromise(promiseOrValue.then((value) => {
        console.log('------------------')
        console.log('promise.then', value, dependencies)
      ...
      // console.log('suspensePromise', suspensePromise)
  ...
  const readAtomState = (version, atom2, force) => {
    ...
    try {
      const promiseOrValue = atom2.read((a) => {
        ...
        // console.log('aState', aState, version)
        if (aState) {
          if ("e" in aState) {
            throw aState.e;
          }
  ...
  const writeAtomState = (version, atom2, update) => {
    ...
    const writeGetter = (a, options) => {
      ...
      // console.log('writeAtomState', aState)
      if ("e" in aState) {
        throw aState.e;
      }
      if ("p" in aState) {
  ...
  const mountAtom = (atom2, initialDependent) => {
    ...
    // console.log('atom2.onMount', atom2.onMount)
  ...
  const mountDependencies = (atom2, atomState, prevReadDependencies) => {
    ...
    // console.log('dependencies', dependencies, atom2)
    dependencies.forEach((a) => {
      const mounted = mountedMap.get(a);
      if (mounted) {
        mounted.t.add(atom2);
        // console.log('mounted222', mounted)
  ...
  const flushPending = (version) => {
    ...
    // console.log('pending', Array.from(pendingMap))
    // console.log('mountedMap', mountedMap)
    console.log('flushPending', Array.from(pendingMap))
    if (version === null)
      console.log('jotai 开始触发页面更新')
    while (pendingMap.size) {
      const pending = Array.from(pendingMap);
      pendingMap.clear();
      pending.forEach(([atom2, prevAtomState]) => {
        const atomState = getAtomState(void 0, atom2);
        // console.log('mountDependencies', atom2)
...
function useAtom(atom2, scope) {
  ...
  const getAtomValue = (0, import_react.useCallback)((version2) => {
    ...
    console.log('getAtomValue', atomState.p, "v" in atomState)
    if ("e" in atomState) {
      throw atomState.e;
    }
    if ("p" in atomState) {
  ...
  const [[version, value, atomFromUseReducer], rerenderIfChanged] = (0, import_react.useReducer)((0, import_react.useCallback)((prev, nextVersion) => {
    const nextValue = getAtomValue(void 0);
